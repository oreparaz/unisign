#!/bin/bash
# This script verifies that the magic placeholder string is preserved in the compiled C binary.
# It compiles the C example and then checks for the presence of the placeholder string.
...
echo -e "${YELLOW}Verifying the placeholder string in the compiled C binary...${NC}"
...
if strings "$BINARY_PATH" | grep -q "$MAGIC_STRING"; then
    ...
    # Use readelf/otool to check what section the magic string is in
    if [[ "$OSTYPE" == "darwin"* ]]; then
        echo "Checking section information (macOS)..."
        otool -l "$BINARY_PATH" | grep -A2 __unisign || echo -e "${YELLOW}Section information not found with otool${NC}"
    else
        echo "Checking section information (Linux/other)..."
        readelf -S "$BINARY_PATH" | grep .note.unisign || echo -e "${YELLOW}Section information not found with readelf${NC}"
    fi
